...
ggplot(top_joueurs, aes(x = reorder(label, -Goals), y = Goals)) +
  ...
  labs(title = "Top 10 des meilleurs buteurs",
       ...
       y = "Nombre de buts") + theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
...
ggplot(top_nation_data, aes(x = Nationality, y = Goals)) +
  theme_minimal() +  
  geom_boxplot(fill = "skyblue", color = "darkblue") +
  coord_cartesian(ylim = c(0, quantile(top_nation_data$Goals, 0.9))) +  # zoom 
  labs(title = "Distribution des buts par nationalité (zoom sur la distribution centrale )",
       x = "Nationalité",
       y = "Nombre de buts")
...
ggplot(data_clean, aes(x = Shooting.accuracy.., y = Goals_per_match)) +
  ...
  labs(
    title = "Précision des tirs vs moyenne de buts par match",
    x = "Précision des tirs (%)",
    y = "Buts par match"
  )
```
...
```{r echo=FALSE}
# Préparation des données : passage au format long
df_long <- data %>%
  group_by(Position) %>%
  summarise(
  ...
  pivot_longer(
    cols = c(Fouls, Yellow.cards, Red.cards),
    names_to = "Sanction",
    values_to = "Moyenne"
  )

# Barplot groupé
ggplot(df_long, aes(x = Sanction, y = Moyenne, fill = Position)) +
  geom_col(position = position_dodge(width = 0.8), width = 0.7) +
  scale_fill_viridis_d(name = "Poste") +
  labs(
...
# Question 7 : Quel type de but est le plus fréquent en Premier League ? (tête, pied droit, pied gauche)

Pour répondre à cette question, nous avons comparé les trois types de buts enregistrés pour chaque joueur : les buts marqués de la tête (Headed goals), du pied droit (Goals with right foot), et du pied gauche (Goals with left foot). L’objectif est d’identifier quelle méthode est la plus utilisée par les joueurs pour marquer.

Nous avons représenté les moyennes de chaque type de but à l’aide d’un graphique en barres groupées.
...
df_goals <- data %>%
  summarise(
    `Headed goal` = mean(Headed.goals, na.rm = TRUE),
    `Right foot` = mean(Goals.with.right.foot, na.rm = TRUE),
    `Left foot` = mean(Goals.with.left.foot, na.rm = TRUE)
  ) %>%
  pivot_longer(cols = everything(), names_to = "GoalType", values_to = "AverageGoals")

# 绘图：条形图
ggplot(df_goals, aes(x = GoalType, y = AverageGoals, fill = GoalType)) +
  geom_bar(stat = "identity", width = 0.6) +
  labs(
    title = "Average Number of Goals by Type",
    x = "Type of Goal",
    y = "Average Goals per Player"
  ) +
...
Résultats observés
Pied droit : La majorité des buts sont marqués du pied droit, ce qui s’explique par la dominance des joueurs droitiers dans le football professionnel.
Pied gauche : Les buts du pied gauche sont un peu moins fréquents mais restent significatifs.
Tête : Les buts de la tête sont les moins nombreux en moyenne.

Conclusion
La frappe du pied droit est de loin la méthode la plus utilisée pour marquer, suivie par le pied gauche. Les buts de la tête restent plus rares et peuvent dépendre davantage du poste et du style de jeu.
Pour approfondir la question, nous sommes curieux de savoir si cela est lié au style du club auquel appartient le joueur et si le style d'entraînement du club a une influence sur les résultats.
...
df_club <- data %>%
  ...
  pivot_longer(cols = -Club, names_to = "GoalType", values_to = "AvgGoals")

# 选出进球最多的前 10 个俱乐部
top_clubs <- df_club %>%
  group_by(Club) %>%
  summarise(Total = sum(AvgGoals)) %>%
  top_n(10, Total) %>%
  pull(Club)
...
# 绘图
ggplot(df_plot, aes(x = GoalType, y = AvgGoals, fill = GoalType)) +
  geom_col() +
  facet_wrap(~ Club) +
  labs(
    title = "Comparison of Goal Types by Club",
    x = "Type of Goal",
    y = "Average Goals per Player"
  ) +
  scale_fill_brewer(palette = "Pastel1") +
...
```
Bien que la tendance générale reste conforme à l’analyse précédente, des clubs comme Crystal Palace et Everton se démarquent par un nombre de buts de la tête légèrement supérieur à la moyenne, ce qui peut suggérer une plus grande dépendance aux centres latéraux ou aux tactiques de jeu aérien. 
En revanche, Manchester City et Tottenham Hotspur se distinguent par un nombre particulièrement élevé de buts marqués du pied droit, ce qui indique que leurs principaux joueurs offensifs sont majoritairement droitiers et présentent une grande efficacité devant le but. 
De manière générale, on observe encore des différences structurelles dans les types de buts selon les clubs, probablement influencées par la répartition des pieds dominants des joueurs, les styles tactiques des équipes ou encore les caractéristiques de leurs joueurs clés.

# Question 8 : Quel est le style de jeu des clubs ? 
L'objectif de cette question est de déceler des caractéristiques qui différencie 
les clubs au niveau du jeu pour cela on va comparer leurs caractéristiques dans chaque facette du jeu : 
- Jeu direct (LongBalls, ThroughBalls)
- Possession (PassesPerMatch)
- Pressing (Recoveries, DuelsWon)
- Tir (ShootingFromDistance)
- Jeu sur les ailes (Crosses, CrossAccuracy)

```{r echo=FALSE}
# Nettoyage des variables nécessaires
club_styles <- club_stats %>%
  mutate(
    Jeu_Direct = (LongBalls + ThroughBalls) / 2,
    Possession = PassesPerMatch,
    Pressing = (Recoveries + DuelsWon) / 2,
    Tir = ShootingFromDistance,
    Ailes = (Crosses + CrossAccuracy) / 2
  ) %>%
  select(Club, Jeu_Direct, Possession, Pressing, Tir, Ailes)

# 2. Normalisation (0-1)
normalized_styles <- as.data.frame(scale(club_styles[,-1],
                                         center = apply(club_styles[,-1], 2, min, na.rm = TRUE),
                                         scale = apply(club_styles[,-1], 2, max, na.rm = TRUE) - apply(club_styles[,-1], 2, min, na.rm = TRUE)))

rownames(normalized_styles) <- club_styles$Club

# 3. Affichage des radar charts : 2x2 par page
par(mfrow = c(2, 2), mar = c(2, 2, 4, 2))  # Ajuste le layout ici (ex: 3x3 pour + clubs par page)

for (club in rownames(normalized_styles)) {
  radar_data <- rbind(rep(1, ncol(normalized_styles)),
                      rep(0, ncol(normalized_styles)),
                      normalized_styles[club, ])
  
  radarchart(radar_data,
             axistype = 1,
             pcol = "blue",
             pfcol = adjustcolor("blue", alpha.f = 0.25),
             plwd = 2,
             cglcol = "grey", cglty = 1,
...
```
On constate que certains club ont un style particulier de jeu comme Fulham et Chelsea sont basés
sur la possession, et d'autre comme Burnley et Crystal-palace basés sur un jeu de pressing et de jeu direct.
D'autres club sont plus équilibrés. Ce qui nous demande à savoir parmi ces clubs quel style de jeu est le plus efficace.
Pour cela, on peut essayer de corréler avec le nombre de buts.

# Question 9: Quels sont les clubs les plus performants offensivement cette saison en Premier League ? (graphique fait par shiny)

![](shiny/shiny_graphique.png)

En croisant les styles de jeu identifiés (possession, pressing, jeu direct...) avec le classement des clubs selon le nombre de buts marqués, on observe que plusieurs approches peuvent mener à l’efficacité offensive, à condition d’être bien exécutées.

## Clubs performants
Manchester City domine nettement : leur style basé sur la possession, le pressing haut et la créativité collective se traduit par une production offensive exceptionnelle.

Liverpool, avec un jeu rapide et intense sur les ailes, confirme l'efficacité d’un pressing fort et d’attaques directes bien menées.

Tottenham adopte un jeu plus vertical, exploitant parfaitement les transitions et la finition de joueurs comme Son ou Kane.

## Surprise
Crystal Palace, 4e, surprend par son efficacité. Son style plus direct et son jeu de contre semblent avoir été exploités au maximum, malgré un profil d'équipe plus modeste.

## Clubs décevants
Arsenal, seulement 9e, montre que la possession seule ne suffit pas sans efficacité dans le dernier tiers.

Chelsea, 10e, illustre une difficulté à transformer ses occasions malgré un effectif offensif. Leur manque de cohérence dans le style de jeu pourrait expliquer cette contre-performance.

## En résumé
Aucun style ne garantit à lui seul le succès offensif. Ce sont la qualité d'exécution, l'adéquation entre le style et les joueurs, et la régularité qui semblent déterminer l'efficacité devant le but.
